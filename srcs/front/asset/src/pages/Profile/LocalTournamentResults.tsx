  ...
  useEffect(() => {
        ...
        try {
            ...
            if (tournamentsInfos)
            {
                for (let i = 0; i < tournamentsInfos.length; i++) {
                    ...
                    const localTournamentGames: Map<number, Array<number>> = parseGamesJson(tournament.games);
                    ...
                    for (let j = 0; j < localTournamentGames.size; j++) {
                        const match = localTournamentGames.get(i);
                        if (!match)
                            continue ;
                        ...
                        for (let k = 0; k < match.length; k++) {
                            ...
                            try {
                                matchPlayer.score = await tournamentContract.getPlayerScore(player);
                                console.info(`Score of player ${player} (${tournament.players[player]}) in game ${j} from tournament ${tournament.id}: ${score}`);
                            } catch (error) {
    ...
    async function getLocalHistory()
    {
        try {
            ...
            console.log('tournamentsInfos', tournamentsInfos);
        } catch (error) {
